...
//        view.translatesAutoresizingMaskIntoConstraints = true
    ...
    func setTitleViewWithTitle(aTitle: String) {
        ...
        if YPConfig.library.options != nil {
            ...
            button.setBackgroundColor(UIColor.white.withAlphaComponent(0.2), forState: .highlighted)
    ...
    func updateUI() {
        if !YPConfig.hidesCancelButton {
            ...
            if #available(iOS 13.0, *) {
                let backButton = UIImage(systemName: "xmark", withConfiguration: UIImage.SymbolConfiguration(pointSize: 18, weight: .medium))
                navigationItem.leftBarButtonItem = UIBarButtonItem(image: backButton, style: .plain, target: self, action: #selector(close))
